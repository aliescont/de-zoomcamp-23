...
from settings import RIDES_ALL_SCHEMA, CONSUME_TOPIC_RIDES_ALL_CSV, TOPIC_WINDOWED_PULOCATIONID_COUNT
...
if __name__ == "__main__":
    ...
    # read_streaming data
    df_consume_stream = read_from_kafka(consume_topic=CONSUME_TOPIC_RIDES_ALL_CSV)
    print(df_consume_stream.printSchema())

    # parse streaming data
    df_rides = parse_ride_from_kafka_message(df_consume_stream, RIDES_ALL_SCHEMA)
    print(df_rides.printSchema())

    sink_console(df_rides, output_mode='append')

    df_trip_count_by_pulocation_id = op_groupby(df_rides, ['PULocationID'])
    df_trip_count_by_pickup_date_pulocation_id = op_windowed_groupby(df_rides, window_duration="10 minutes",
                                                                 slide_duration='5 minutes')

    # write the output out to the console for debugging / testing
    sink_console(df_trip_count_by_pulocation_id)
    # write the output to the kafka topic
    df_trip_count_messages = prepare_df_to_kafka_sink(df=df_trip_count_by_pickup_date_pulocation_id,
                                                      value_columns=['count'], key_column='PULocationID')
    kafka_sink_query = sink_kafka(df=df_trip_count_messages, topic=TOPIC_WINDOWED_PULOCATIONID_COUNT)
